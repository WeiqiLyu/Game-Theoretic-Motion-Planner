...
class SE_IBR_Planner:  
    ...
    def se_ibr(
        ...
        for index in range(num_veh):
            ...
            if self.vehicles[name].no_dynamics:
                agents_traj_xcurv, agents_traj_xglob = vehicles[name].get_trajectory_nsteps(  
                    time,                              #t0
        ...
        print("se_ibr_local planner solver time: {}".format(solver_time))
    ...
    def solve_se_ibr(self):
        ...
        for iter in range(1, L+1):
            ...
            for index in range(num_veh):
                self.solution_xglob[iter,index, :, :] = dict_traj[index]  
    ...
    def solve_optimization_problem(self, num_iter,pos_index, dict_traj, dict_xpoly, dict_ypoly,dict_solve_time,dict_cost): 
        ...
        alpha = self.alpha * exp(-(num_iter-1))
        mue = self.mue 
        track = self.track
        ...
        beta = np.zeros((num_veh,num_horizon_planner+1,2))
        ...
        for i in range(num_horizon_planner + 1):    
            ...
            for index in range(num_veh):
                if   index == pos_index:
                    ...
                    # linear approximation of sensitivity enhanced term
                    beta_norm = np.linalg.norm([obs_traj_xglob[0,i]-vehicles_infos_xglob[0,i],obs_traj_xglob[1,i]-vehicles_infos_xglob[1,i]])
                    beta[index,i,0] = (obs_traj_xglob[0,i]-vehicles_infos_xglob[0,i])/beta_norm
                    beta[index,i,1] = (obs_traj_xglob[1,i]-vehicles_infos_xglob[1,i])/beta_norm
                    cost_se_ibr -= alpha*(beta[index,i,0]*x[0,i] + beta[index,i,1]*x[1,i])*mue
        ...
        cost_se_ibr -= s_lin_N#/(1 - curv_track[num_horizon_planner]*ey[num_horizon_planner]) 
        ...
        print("se_ibr planner solver time: {}".format(solve_time))
        ...
        dict_cost[pos_index] = cost_se_ibr