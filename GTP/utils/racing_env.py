...
def get_global_position(lap_length, width, point_and_tangent, s, ey):
    ...
    index = np.all(   
        [
            [s >= point_and_tangent[:, 3]],  #s>=cumulative s
    ...
    if point_and_tangent[i, 5] == 0.0:       # If segment is a straight line
        ...
        xf = point_and_tangent[i, 0]         # final point
        yf = point_and_tangent[i, 1]    
        xs = point_and_tangent[i - 1, 0]     # start point
        ys = point_and_tangent[i - 1, 1]
        psi = point_and_tangent[i, 2]   

        # Compute the segment length
        deltaL = point_and_tangent[i, 4]     # deltaL = segment length
        reltaL = s - point_and_tangent[i, 3] # relta = s-cumulative_s
        ...
        r = 1 / point_and_tangent[i, 5]      # Extract curvature  r=1/signed curvature
        # Extract angle of the tangent at the initial point (i-1)
        ang = point_and_tangent[i - 1, 2]    # ang = psi_i-1
...
def get_orientation(lap_length, point_and_tangent, s, ey):
    # wrap s along the track
    s_tolerance = 0.001
    while s > lap_length:
        s = s - lap_length
    ...
    if point_and_tangent[i, 5] == 0.0:  # If segment is a straight line
        ...
        psi = wrap(angle + direction * spanAng + np.pi / 2)
...
def get_local_position(lap_length, width, point_and_tangent, x, y, psi):
    ...
    for i in range(0, point_and_tangent.shape[0]):  
        ...
        if point_and_tangent[i, 5] == 0.0:  # If segment is a straight line
            ...
            if (la.norm(np.array([xs, ys]) - np.array([x, y]))) == 0:   # in the initial position
                s = point_and_tangent[i, 3]
                ey = 0
                CompletedFlag = 1

            elif (la.norm(np.array([xf, yf]) - np.array([x, y]))) == 0: # in the final position
                ...
                if ( 
                    ...
                    if np.abs(ey) <= width:   # this width is track half-width
                        CompletedFlag = 1
        else:
            xf = point_and_tangent[i, 0]
            yf = point_and_tangent[i, 1]
            ...
            if (la.norm(np.array([xs, ys]) - np.array([x, y]))) == 0:   # in the initial position
                ...
                s = point_and_tangent[i, 3]
                CompletedFlag = 1
            elif (la.norm(np.array([xf, yf]) - np.array([x, y]))) == 0:  # in the fianl position
                s = point_and_tangent[i, 3] + point_and_tangent[i, 4]
                ey = 0
                ...
                arc1 = point_and_tangent[i, 4] * point_and_tangent[i, 5]  # arc1 = segment length * signed curavature
...
def plot_track(ax, lap_length, width, point_and_tangent, center_line):
    ...
    for i in range(0, num_track_points):
        points_out[i, :] = get_global_position(
            ...
            width,
            point_and_tangent,
            i / float(num_sampling_per_meter),  #s=i/100 [m]
            width,                              #ey=width
        )
...
class ClosedTrack:
    ...
    def __init__(self, spec, track_width=0.8, discre = 100):
        """Initialization
    ...
    def get_orientation(self, s, ey):
        psi = get_orientation(self.lap_length, self.point_and_tangent, s, ey)
        return psi
